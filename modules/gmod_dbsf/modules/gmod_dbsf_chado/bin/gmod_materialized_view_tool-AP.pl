...
=head1 COMMAND-LINE OPTIONS
 ...
 --notimer			  Forgoe the timer in favour of performance
...
my ( $DBPROFILE, $STATUS, $LIST, $NAME, $DEMATERIALIZE, $CREATE_VIEW,
    $UPDATE_VIEW, $AUTOMATIC, $HELP, $NOTIMER );
...
sub update_mv {
    ...
    unless ($row) {
        print"The MV with the name '$name' does not exist.  Here is a list of existing MV's: ";
        print join( ", ", @NAMES );
        print "\n";
        return;
    }
    print "Retrieving MV data\n";
    my ( $mv_schema, $mv_table, $mv_specs ) = ( $row->{mv_schema}, $row->{mv_table}, $row->{mv_specs} );
    ...
    unless ($exists) {
        print "Creating materialized view '$name' for the first time at $mv_schema.$mv_table...\n";
    ...
    my $query = $row->{query};
    my $eta;
    sleep(3);	# allow cancellation within 3'' before initiating expensinve count
    if (!$NOTIMER){
    ...
    while ( my $entry = $sth->fetchrow_hashref() ) {
        ...
        $insq->execute(@values);
        $i++;
        $eta->update_and_print($i) if (!$NOTIMER && $i=~/0$/);
    }

    $dbh->do("SET SEARCH_PATH=$mv_schema");
    print "\nUpdate of MV '$name' successful. Commiting...\n";
    $dbh->commit();
    $dbh->{AutoCommit} = 1;
    print "Vacuuming...\n";
    $dbh->do("VACUUM FULL FREEZE ANALYZE $mv_schema.$mv_table");
    ...
    my $timeupq = $dbh->prepare("UPDATE $SCHEMA.$TABLE SET last_update=NOW() WHERE name=?");
