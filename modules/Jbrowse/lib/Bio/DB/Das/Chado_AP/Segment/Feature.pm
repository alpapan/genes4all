...
Bio::DB::Das::Chado_AP::Segment::Feature

=head1 SYNOPSIS

See L<Bio::DB::Das::Chado_AP>.
...
package Bio::DB::Das::Chado_AP::Segment::Feature;

use strict;

use Bio::DB::Das::Chado_AP::Segment;
use Bio::SeqFeatureI;
use Bio::Root::Root;
use Bio::LocationI;
use Data::Dumper;
...
@ISA = qw(Bio::DB::Das::Chado_AP::Segment Bio::SeqFeatureI
          Bio::Root::Root);
$VERSION = 0.30;
...
=head2 new
 ...
 Title   : new
 Usage   : $f = Bio::DB::Das::Chado_AP::Segment::Feature->new(@args);
 Function: create a new feature object
 Returns : new Bio::DB::Das::Chado_AP::Segment::Feature object
 Args    : see below
 Status  : Internal

This method is called by Bio::DB::Das::Chado_AP::Segment to create a new
feature using information obtained from the chado database.

The 11 arguments are positional:

  $factory      a Bio::DB::Das::Chado_AP adaptor object (or descendent)
...
Implemented in Bio::DB::Das::Chado_AP::Segment
...
return all dbxref for feature

=cut

sub dbxref_table {
 my $self = shift;
 return $self->{'dbxref_table'} = shift if defined($_[0]);
 my $dbh = $self->factory->dbh;
 my $feature_id = $self->feature_id;
 my $sth = $dbh->prepare(
        "SELECT d.dbxref_id as id,accession,db.name as db,urlprefix FROM feature_dbxref fd JOIN dbxref d ON d.dbxref_id=fd.dbxref_id "
        ." JOIN db ON d.db_id=db.db_id WHERE feature_id=?"
    );
  $sth->execute($feature_id );
  my $dbxref_ref=$sth->fetchall_hashref('accession');
  $sth->finish;
  return $self->{'dbxref_table'} = $dbxref_ref;     
}
...
Bio::DB::Das::Chado_AP::Segment::Feature implements the Bio::SeqFeatureI
...
=head2 end()
  ...
  Title   : end
  Function: inherited, L<Bio::DB::Das::Chado_AP::Segment>
...
=head2 target()
  ...
  Function: returns a Bio::DB::Das::Chado_AP::Segment that corresponds
            to the target of a similarity pair
  Returns : a Bio::DB::Das::Chado_AP::Segment object
  Args    : none
...
# on target per feature, Bio::Graphics::Browser doesn't support it
  ...
  if ($$hashref{'name'}) {
      my $segment = Bio::DB::Das::Chado_AP::Segment->new(
                $$hashref{'name'},
                $self->factory,
                $$hashref{'fmin'}+1,
                $$hashref{'fmax'},
...
=head2 sub_SeqFeature()
 ...
 Returns : a list of Bio::DB::Das::Chado_AP::Segment::Feature objects
...
=head2 add_subfeature()
 ...
 Returns : nothing
 Args    : a Bio::DB::Das::Chado_AP::Segment::Feature object
 Status  : Public
...
sub add_subfeature {
  ...
  return undef unless $subfeature->isa('Bio::DB::Das::Chado_AP::Segment::Feature');
...
=head2 clone()
 ...
 Returns : a new Bio::DB::Das::Chado_AP::Segment::Feature object
 Args    : none
 Status  : Public
