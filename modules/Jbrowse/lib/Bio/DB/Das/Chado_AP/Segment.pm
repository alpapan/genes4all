...
Bio::DB::Das::Chado_AP::Segment - DAS-style access to a chado database

=head1 SYNOPSIS

  # Get a Bio::Das::SegmentI object from a Bio::DB::Das::Chado_AP database...
...
Bio::DB::Das::Chado_AP::Segment is a simplified alternative interface to
...
landmarks.  Each Bio::DB::Das::Chado_AP::Segment object ("segment") corresponds
...
=cut
package Bio::DB::Das::Chado_AP::Segment;
use strict;
use Carp qw(carp croak cluck confess);
use Bio::Root::Root;
use Bio::SeqI;
use Bio::Das::SegmentI;
use Bio::DB::Das::Chado_AP;
use Bio::DB::Das::Chado_AP::Segment::Feature;
...
@ISA     = qw(Bio::Root::Root Bio::SeqI Bio::Das::SegmentI Bio::DB::Das::Chado_AP);
$VERSION = 0.30;
...
"The query for $name returned multiple segments\nPlease call in a list context to get them all";
		...
		if ( $landmark_feature_id == $srcfeature_id ) {
			...
			warn "skipping feature_id $$hash_ref{feature_id}"
			  if (     DEBUG
				   and $$hash_ref{'is_obsolete'}
				   and !$self->factory->allow_obsolete );
			...
			if ( defined($interbase_start) and $interbase_start < 0 ) {
				$self->warn(
						   "start value ($interbase_start) less than zero," . " resetting to zero" )
				  if DEBUG;
				$base_start      = 1;
				$interbase_start = 0;
			}
			if ( defined($stop) and defined($length) and $stop > $length ) {
				$self->warn(
					 "end value ($stop) greater than length ($length)," . " truncating to $length" )
				  if DEBUG;
				$stop = $length;
			}
			...
			my $type_obj = Bio::DB::GFF::Typename->new( $type, $source );
			$self->type($type_obj);
...
sub _search_by_name {
	...
	if ($feature_id) {
		...
		$sth = $factory->dbh->prepare( "
             select name,feature_id,seqlen from feature
             where name = $quoted_name $obsolete_part " );
	}
	...
	if ( $rows_returned == 0 ) {    #look in synonym for an exact match
		...
		if ( $self->factory->use_all_feature_names() ) {
			...
			if ($fulltext) {
				...
				$optional_full_text = "afn.name = $quoted_name $where_part";
			}
      ...
      " );
			...
			if ($fulltext) {
				...
				$full_text_options = "s.synonym_sgml = $quoted_name $where_part";
			}
			$isth = $factory->dbh->prepare( "
      ...
      " );
		...
		if ( $rows_returned == 0 ) {    #look in dbxref for accession number match
			...
			if ($fulltext) {
				$full_text_option =
				  "d.searchable_accession @@ plainto_tsquery($quoted_name) $where_part";
			} else {
				$full_text_option = "d.accession = $quoted_name $where_part";
			}
			$isth = $factory->dbh->prepare( "
...
=head2 type
  ...
  Function: used to be alias of class() for backward compatibility,
            now behaves the same as Bio::DB::Das::Chado_AP::Segment::Feature->type
...
sub features {
	...
	my (
		 $types,    $type_placeholder, $attributes, $rangetype, $iterator,
		 $callback, $base_start,       $stop,       $seq_id,    $end
	);
	...
	warn "types are @$types\n" if ( defined $types and DEBUG );
	warn Dumper $factory if DEBUG;
	$factory ||= $self->factory();
	my $feat = Bio::DB::Das::Chado_AP::Segment::Feature->new();
	my @features;
	...
	if ( !$feature_id ) {
		...
		warn "sql_types: $sql_types" if DEBUG;
		...
		if ( !$srcfeature_id && defined($seq_id) ) {
			...
			my $srcfeature_query = "SELECT feature_id FROM feature where uniquename = ? ";
			$srcfeature_query .= "and organism_id = " . $factory->organism_id
			  if $factory->organism_id;
			warn "Looking for source feature with \n $srcfeature_query : $seq_id" if DEBUG;
			my $srcf_query_handle = $factory->dbh->prepare($srcfeature_query);
			$srcf_query_handle->execute( $seq_id) ;
			($srcfeature_id) = $srcf_query_handle->fetchrow_array;
		}
		warn "found srcfeature_id:$srcfeature_id" if DEBUG;
	}

#AP would be much faster to break this query: get feature_id et al via srcfeature and then use feature_id to get other data
##  my $select_part = "select distinct f.name,fl.fmin,fl.fmax,fl.strand,fl.phase,"
	...
	my $src_query =
"SELECT DISTINCT fmin,fmax,strand,phase,locgroup,srcfeature_id,feature_id from featureloc WHERE rank=0 ";
	$src_query .= " AND feature_id = $feature_id " if $feature_id;
	my $feature_query =
	    "SELECT f.name,f.type_id,f.uniquename,f.feature_id,fd.dbxref_id FROM feature f "
	  . " join feature_dbxref fd ON (f.feature_id = fd.feature_id AND fd.dbxref_id in (SELECT dbxref_id FROM dbxref WHERE db_id="
	  . $factory->gff_source_db_id . "))";
	$feature_query .= " AND $sql_types " if defined($sql_types);
	my $analysis_query = "SELECT significance as score from analysisfeature WHERE feature_id=?";
	if ($feature_id) {
		...
		if ( defined($refclass_feature_id) and defined($srcfeature_id) ) {
			$src_query .= " AND srcfeature_id = $refclass_feature_id ";
		} elsif ($refclass) {
			...
			$sth->execute( $feature_id, $refclass )
			  or $self->throw("refclass_srcfeature query failed");
			my $hashref = $sth->fetchrow_hashref();
			my $srcfeature_id = $hashref->{srcfeature_id} || undef;
			$src_query .= " AND srcfeature_id = $srcfeature_id " if ( defined($srcfeature_id) );
			$sth->finish;
		}
	} else {
		my $featureslice;
		...
		$src_query =~ s/from featureloc /from $featureslice /;
		$src_query .= " AND srcfeature_id = $srcfeature_id " if defined($srcfeature_id);
	}
	...
	$feature_query .= " AND f.organism_id = " . $self->factory->organism_id
	...
	if ( $factory->recursivMapping && !$feature_id ) {
		my $src_from = $src_query;
		$src_from =~ s/featureslice/recurs_featureslice/g;
		$src_query .= "UNION\n" . $src_from;
	}
	$src_query .= "ORDER by fmin";

	#END Recursive Mapping
	#$factory->dbh->do("set enable_seqscan=0");

	#  $factory->dbh->do("set enable_hashjoin=0");
	warn "Segment->features src_query:$src_query" if DEBUG;
	my $src_query_prepared = $factory->dbh->prepare($src_query);
	$src_query_prepared->execute or $self->throw("srcfeature query failed");
	#$factory->dbh->do("set enable_seqscan=1");
	$feature_query .= " AND f.feature_id=?";
	$feature_query .= " ORDER BY f.type_id";
	my $feature_query_prepared  = $factory->dbh->prepare($feature_query);
	my $analysis_query_prepared = $factory->dbh->prepare($analysis_query);
	...
	while ( my $hashref = $src_query_prepared->fetchrow_hashref ) {
		
		#$$hashref{'fmin'};
		#$$hashref{'fmax'};
		my $srcfeature_id = $$hashref{'srcfeature_id'};
		my $strand        = $$hashref{'strand'};
		my $phase         = $$hashref{'phase'};
		my $seqlen        = $$hashref{'seqlen'};
        #warn "Executing feature_id ".$$hashref{'feature_id'}." with feature_query: $feature_query" if DEBUG;
		$analysis_query_prepared->execute($$hashref{'feature_id'});
		my $analysis_data_ref = $analysis_query_prepared->fetchrow_hashref;
		my $score = $$analysis_data_ref{'score'};
		$analysis_query_prepared->finish();
		
		$feature_query_prepared->execute($$hashref{'feature_id'}) or $self->throw("feature details query failed");
		my $feature_data_ref = $feature_query_prepared->fetchrow_hashref;
		$feature_query_prepared->finish() unless $feature_data_ref;
		next unless $feature_data_ref ;
        warn "Executed feature_id ".$$hashref{'feature_id'}." with feature_query: $feature_query" if DEBUG;
		warn "sequence length is $seqlen" if (DEBUG && defined($seqlen));
		my $is_obsolete = $$feature_data_ref{'is_obsolete'};
		my $dbxref_id   = $$feature_data_ref{'dbxref_id'};
		my $type_id     = $$feature_data_ref{'type_id'}||next;
		my $name        = $$feature_data_ref{'name'};
		my $uniquename  = $$feature_data_ref{'uniquename'};
		$feature_query_prepared->finish();
		warn "dbstart:$$hashref{'fmin'}, dbstop:$$hashref{'fmax'} seqlen:$$hashref{seqlen}\n" if DEBUG;
		warn "start:$base_start, stop:$stop\n" if DEBUG;
		warn "skipping feature_id $$hashref{feature_id} because it is obsolete"
            if (DEBUG and
                $is_obsolete and !$self->factory->allow_obsolete);
    next if ($is_obsolete and !$self->factory->allow_obsolete);
    ...
    warn "NEW base_start:$base_start, end:$stop" if DEBUG;

    my $source = $factory->dbxref2source($dbxref_id) || "" ;
    my $type   = Bio::DB::GFF::Typename->new(
                     $factory->term2name($type_id),
                     $source);

    if (defined $seqlen) { #this is a srcfeature
     $feat = Bio::DB::Das::Chado_AP::Segment::Feature->new(
                       $factory,
                       undef,
                       undef,
                       $base_start,$stop,
	...
	$src_query_prepared->finish;
	if ($iterator) {
		warn "using Bio::DB::Das::Chado_APIterator\n" if DEBUG;
		return Bio::DB::Das::Chado_APIterator->new( \@features ) if @features;
	} elsif (wantarray) {
		return @features;
	} elsif ( @features > 0 ) {
		return \@features;
...
Bio::DB::Das::Chado_AP::Segment->sub_SeqFeatures are avoided, but this
...
sub _features2level() {
	...
	my (
		 $types,    $type_placeholder, $attributes, $rangetype,  $iterator,
		 $callback, $base_start,       $stop,       $feature_id, $factory
	);
	if ( $_[0] and $_[0] =~ /^-/ ) {
		(
		   $types,    $type_placeholder, $attributes, $rangetype,  $iterator,
...
#each feature is spaned over several tuples, each of which store a different SUBfeature (only one tuple if no subfeat of course)
	while ( my $hashref = $sth->fetchrow_hashref ) {
		...
		if ( !defined( $feat->feature_id ) || $feat->feature_id != $$hashref{feature_id} ) {
			...
			my $type =
			  Bio::DB::GFF::Typename->new( $factory->term2name( $$hashref{type_id} ), $source );
			$feat = Bio::DB::Das::Chado_AP::Segment::Feature->new(
				$factory,
				$feature_id ? undef : $self,    #only give the segment as the
				                                # parent if the feature_id wasn't
...
"Created Feature obj $$hashref{name}][[$$hashref{feature_id}][$$hashref{'srcfeature_id'}]\n"
		...
		if ( $$hashref{sfeature_id} ) {
			...
			my $source = $factory->dbxref2source( $$hashref{dbxref_id} ) || "";
			my $type =
			  Bio::DB::GFF::Typename->new( $factory->term2name( $$hashref{stype_id} ), $source );
			my $subFeat = Bio::DB::Das::Chado_AP::Segment::Feature->new(
				$factory,
				$feat,
				$feature_id
				? $factory->srcfeature2name( $$hashref{'srcfeature_id'} )
...
=head2 get_feature_stream
  ...
  Returns : A Bio::DB::Das::Chado_APIterator object
...
sub get_feature_stream {
	...
	return Bio::DB::Das::Chado_APIterator->new($features) if $features;
	return Bio::DB::Das::Chado_APIterator->new( [] );
}
