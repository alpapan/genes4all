...
Bio::DB::Das::Chado_AP - DAS-style access to a chado database

=head1 SYNOPSIS

  # Open up a feature database
                 $db    = Bio::DB::Das::Chado_AP->new(
...
use constant SEGCLASS => 'Bio::DB::Das::Chado_AP::Segment';
...
=head2 new
 ...
 Returns : a new Bio::DB::Das::Chado_AP object
 Args    :
...
sub feature_summary {
    ...
    my @features = ($feature);
    return $iterator
           ? Bio::DB::Das::Chado_APIterator->new(\@features) 
           : $feature;
}
...
sub get_seq_stream {
    ...
    return Bio::DB::Das::Chado_APIterator->new(\@features);
...
=head2 get_feature_by_alias, get_features_by_alias 
 ...
 Returns : a list of Bio::Das::Chado_AP::Segment::Feature objects
 Args    : See below
  ...
  if ( $operation eq 'by_alias') {
   if ($self->use_all_feature_names()) {
    ...
    if ($self->fulltext) {
        ...
        $alias_only_where = "where afn.name = ?";
    }
  ...
  while (my $feature_id_ref = $sth->fetchrow_hashref) {
    ...
    while (my $hashref = $isth->fetchrow_hashref) {
      ...
      if ($self->refclass && $$hashref{type_id} == $self->refclass) {
          ...
          my $f = Bio::DB::Das::Chado_AP::Segment->new($$hashref{'name'},
                                                    $self,
                                                    1,$$hashref{'seqlen'},
                                                    $$hashref{'uniquename'},
                                                    undef,
      ...
      if ($$hashref{'srcfeature_id'} != $old_srcfeature_id) {
        ...
        $parent_segment =
             Bio::DB::Das::Chado_AP::Segment->new($$src_name{'name'},$self,undef,undef,undef,undef,$$hashref{'srcfeature_id'});
...
package Bio::DB::Das::Chado_APIterator;
