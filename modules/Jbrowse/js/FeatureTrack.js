...
FeatureTrack.prototype.loadSuccess = function(trackInfo) {
	...
	for ( var i = 0; i < this.histogramMeta.length; i++) {
		this.histogramMeta[i].lazyArray = new LazyArray(
				this.histogramMeta[i].arrayParams, this.trackBaseUrl);
	}
		...
		* density;
		...
		if (cc.featureCallback) {
			try {
				...
				console.log("eval failed for featureCallback on track "
						+ this.name + ": " + cc.featureCallback);
			}
		}
	}
	...
	this.onFeatureLabelClick = function(event) {
		...
		var pMenu = new dijit.Menu({
			targetNodeIds: ["text_"+feat[fields["name"]] ],
			//leftClickToOpen: true,
		});
		pMenu.addChild(new dijit.MenuItem({
			label: "Feature "+feat[fields["name"]],
		}));
		if (typeof feat[fields["dbxref_table"]] !== undefined){
			for(var dbxref in feat[fields["dbxref_table"]]){
				if (feat[fields["dbxref_table"]][dbxref].db=="GFF_source"){continue;}
				var score_label;
				if (typeof feat[fields["score"]] !== undefined){
					score_label = 'Significant hit with <b>'+feat[fields["dbxref_table"]][dbxref].accession+'</b> with a score of '+feat[fields["score"]];
				}else{
					score_label = 'Significant hit with <b>'+feat[fields["dbxref_table"]][dbxref].accession+'</b>';
				}
				pMenu.addChild(new dijit.MenuItem({
					label: score_label,
					title: "From database "+feat[fields["dbxref_table"]][dbxref].db
				}));
				if (typeof feat[fields["dbxref_table"]][dbxref].urlprefix !==undefined){
					var rurl = feat[fields["dbxref_table"]][dbxref].urlprefix+ dbxref;
					pMenu.addChild(new dijit.MenuItem({
		...
		if (trackInfo.urlTemplate){
			var featUrl = trackInfo.urlTemplate.replace(/\{([^}]+)\}/g, '');
			if (typeof featUrl != 'undefined' ){
				var lurl = featUrl+feat[fields["name"]];
				pMenu.addChild(new dijit.MenuItem({
					label: "Query internal database",	// must fetch title from json
					iconClass: "dijitIconSearch",
					onClick: function() {
						window.open(lurl);   // new
						// window
					}
				}));
			}
	...
	this.onFeatureClick = function(event) {
		...
		var pMenu2 = new dijit.Menu({
			targetNodeIds: [feat[fields["name"]] ],
			//leftClickToOpen: true,
		});
		...
		if (typeof feat[fields["dbxref_table"]] !== undefined){
			for(var dbxref in feat[fields["dbxref_table"]]){
				...
				if (feat[fields["dbxref_table"]][dbxref].urlprefix !==undefined){
					var rurl = feat[fields["dbxref_table"]][dbxref].urlprefix + dbxref;
					pMenu2.addChild(new dijit.MenuItem({
						label: "Query reference database" ,
						iconClass: "dijitEditorIcon dijitEditorIconTabIndent",
...
FeatureTrack.prototype.setViewInfo = function(genomeView, numBlocks, trackDiv,
		labelDiv, widthPct, widthPx, scale) {
	Track.prototype.setViewInfo.apply(this, [ genomeView, numBlocks, trackDiv,
	                                          labelDiv, widthPct, widthPx, scale ]);
	this.setLabel(this.key);
};

FeatureTrack.prototype.fillHist = function(blockIndex, block, leftBase,
		rightBase, stripeWidth) {
	...
	for ( var i = 0; i < this.histStats.length; i++) {
		if (this.histStats[i].bases >= bpPerBin) {
			// console.log("bpPerBin: " + bpPerBin + ", histStats bases: " +
			// this.histStats[i].bases + ", mean/max: " +
			// (this.histStats[i].mean / this.histStats[i].max));
			logScale = ((this.histStats[i].mean / this.histStats[i].max) < .01);
			pxPerCount = 100 / (logScale ? Math.log(this.histStats[i].max)
	...
	if ((binCount > .9) && (Math.abs(binCount - Math.round(binCount)) < .0001)) {
		...
		histogramMeta.lazyArray.range(firstServerBin, firstServerBin
				+ (binCount * this.numBins), function(i, val) {
		...
		// make our own counts
		this.features.histogram(leftBase, rightBase, this.numBins,
				makeHistBlock);
	}
};
...
FeatureTrack.prototype.endZoom = function(destScale, destBlockBases) {
	if (destScale < this.histScale) {
		this.setLabel(this.key + "<br>per "
				+ Math.round(destBlockBases / this.numBins) + "bp");
	} else {
		this.setLabel(this.key);
	}
	this.clear();
...
FeatureTrack.prototype.fillBlock = function(blockIndex, block, leftBlock,
		rightBlock, leftBase, rightBase, scale, stripeWidth, containerStart,
		containerEnd) {
	...
	if (scale < this.histScale) {
		...
		this.fillFeatures(blockIndex, block, leftBlock, rightBlock, leftBase,
	...
	for ( var i = 0; i < overlaps.length; i++) {
		...
		if (sourceSlot && sourceSlot.feature) {
			if ((sourceSlot.layoutEnd > destLeft)
				...
				var featDiv = this.renderFeature(sourceSlot.feature,
						overlaps[i].id, destBlock, scale, containerStart,
						containerEnd);
...
FeatureTrack.prototype.fillFeatures = function(blockIndex, block, leftBlock,
		rightBlock, leftBase, rightBase, scale, containerStart, containerEnd) {
	// arguments:
	...
	var featCallback = function(feature, path) {
		...
		// console.log("ID " + uniqueId + (layouter.hasSeen(uniqueId) ? "
		// (seen)" : " (new)"));
		if (layouter.hasSeen(uniqueId)) {
			// console.log("this layouter has seen " + uniqueId);
			return;
		}
		var featDiv = curTrack.renderFeature(feature, uniqueId, block, scale,
				containerStart, containerEnd);
	...
	this.features.iterate(startBase, endBase, featCallback, function() {
...
FeatureTrack.prototype.measureStyles = function() {
	...
	heightTest.className = this.className + " plus-" + this.className
	+ " plus-" + this.className + "1";
	if (this.featureCss)
		heightTest.style.cssText = this.featureCss;
	heightTest.style.visibility = "hidden";
	if (Util.is_ie6)
...
FeatureTrack.prototype.renderFeature = function(feature, uniqueId, block,
	...
	if (this.arrowheadClass) {
		...
		case 1:
			featureEnd += (this.plusArrowWidth / scale);
			break;
		case -1:
			featureStart -= (this.minusArrowWidth / scale);
			break;
		}
	}
	...
	if (scale > this.labelScale) {
		...
		.max(
				...
				+ (((fields["name"] && feature[fields["name"]]) ? feature[fields["name"]].length
						: 0) * (this.nameWidth / scale)));
	}
	featureEnd += Math.max(1, this.padding / scale);

	var levelHeight = this.glyphHeight
	+ 2
	+ ((fields["name"] && (scale > this.labelScale)) ? this.nameHeight
			: 0);

	var top = block.featureLayout.addRect(uniqueId, featureStart, featureEnd,
			levelHeight);

	var featDiv;
	var featUrl = this.featureUrl(feature);
	featDiv = document.createElement("div");
	featDiv.onmousedown = this.onFeatureClick;
	featDiv.id = feature[fields["name"]];
	...
	switch (feature[fields["strand"]]) {
	case 1:
		featDiv.className = "plus-" + this.className;
		break;
	case 0:
	case null:
	case undefined:
		featDiv.className = this.className;
		break;
	case -1:
		featDiv.className = "minus-" + this.className;
		break;
	}
	...
	var displayStart = Math.max(feature[fields["start"]], containerStart);
	var displayEnd = Math.min(feature[fields["end"]], containerEnd);
	var blockWidth = block.endBase - block.startBase;
	featDiv.style.cssText = "left:"
	...
	if (this.featureCallback)
		this.featureCallback(feature, fields, featDiv);
	...
	if (this.arrowheadClass) {
		...
		case -1:
			ah.className = "minus-" + this.arrowheadClass;
			ah.style.cssText = "left: " + (-this.minusArrowWidth)
			+ "px; top: 0px;";
			featDiv.appendChild(ah);
			break;
		}
	}
	if ((scale > this.labelScale) && fields["name"] && feature[fields["name"]]) {

		var labelDiv=document.createElement("div");
		labelDiv.onmousedown = this.onFeatureLabelClick;
		labelDiv.className = "feature-label";
		labelDiv.id='text_'+feature[fields["name"]];
		labelDiv.appendChild(document.createTextNode(feature[fields["name"]]));
		labelDiv.style.cssText = "left: "
	...
	if (fields["subfeatures"] && (scale > this.subfeatureScale)
			&& feature[fields["subfeatures"]]
	&& feature[fields["subfeatures"]].length > 0) {

		for ( var i = 0; i < feature[fields["subfeatures"]].length; i++) {
			this
			.renderSubfeature(feature, featDiv,
					feature[fields["subfeatures"]][i], displayStart,
					displayEnd);
		}
	}
...
FeatureTrack.prototype.featureUrl = function(feature) {
	...
	var fields = this.fields;
	if (this.urlTemplate) {
		var href = this.urlTemplate.replace(/\{([^}]+)\}/g, function(match,
				group) {
...
FeatureTrack.prototype.renderSubfeature = function(feature, featDiv,
		subfeature, displayStart, displayEnd) {
	...
	if (this.subfeatureClasses) {
		...
		case 1:
			subDiv.className = "plus-" + className;
			break;
		case 0:
		case null:
		case undefined:
			...
			break;
		case -1:
			subDiv.className = "minus-" + className;
			break;
		}
